...
- **`DGT-ARC-010`**{ #dgt-arc-010 } — For components developed and packaged **at DIGITALL** for later deployment into customer
...
- **`DGT-ARC-020`**{ #dgt-arc-020 } — Once chosen for a project, it does not change — schema name prefixes cannot be renamed after
...
**`DGT-ARC-030`**{ #dgt-arc-030 } — We follow Microsoft's customization guidance by default, and deviate only where there's a
...
**`DGT-ARC-040`**{ #dgt-arc-040 } — Before installing a special-purpose third-party solution into the Power Platform, check
