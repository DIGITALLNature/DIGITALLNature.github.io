...
- **`DGT-ARC-040`**{ #dgt-arc-040 } — For components developed and packaged **at DIGITALL** for later deployment into customer
...
- **`DGT-ARC-050`**{ #dgt-arc-050 } — Once chosen for a project, it does not change — schema name prefixes cannot be renamed after
...
**`DGT-ARC-060`**{ #dgt-arc-060 } — We follow Microsoft's customization guidance by default, and deviate only where there's a
...
**`DGT-ARC-070`**{ #dgt-arc-070 } — Before installing a special-purpose third-party solution into the Power Platform, check
