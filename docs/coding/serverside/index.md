...
- [DIGITALL Assembly (Digitall.APower)](digitall-assembly.md) — our base classes and
  abstraction layer.
...
```mermaid
flowchart LR
    A[".csproj\n(plugin package)"] --> B["Digitall.APower\nbase classes"]
    B --> C["Plugin / Custom API\nclasses"]
    C --> D["[PluginRegistration] /\n[CustomApiRegistration]\nattributes"]
    D --> E["dgtp push\n→ .nupkg → Dataverse"]
```

A project references **`Digitall.APower`** for the base plugin/Custom API/workflow abstraction,
