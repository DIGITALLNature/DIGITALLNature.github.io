...
| Managed Environment | A Dataverse environment with governance features (DLP enforcement, pipelines eligibility, usage insights) enabled by an administrator. |
...
| Solution Checker | Microsoft's static analysis tool for Dataverse solutions (`pac solution check`), used as a quality gate. |
